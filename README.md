...
The dataset is *David Copperfield* by Charles Dickens — Project Gutenberg eBook [#766](https://www.gutenberg.org/ebooks/766), United States public domain. `pg766.txt`, `cleaned_chapters/` and `cleaned_data/cleaned_text.txt` are no longer committed to this repository (`A-33`) — see [Corpus Reproducibility Script](#corpus-reproducibility-script) for how to regenerate what the training notebook needs. The figures below were captured, and independently re-verified, against the corpus while it was still committed, and against its tracked replacement (`scripts/pg766_source.txt`), which is byte-for-byte identical to it (same SHA-256: `5b3e58058f9bb67fe66e1de92bd1d28b24fa02eade777042726a8a2f3195144e`) — confirmed directly against its own Project Gutenberg header (`Title: David Copperfield` / `Author: Charles Dickens`) and against the notebooks' own "Dataset Statistics" cells, which load and print the same header when run.
...
- **Chapters:** 64, extracted from the raw text — regenerate with `scripts/prepare_corpus.py` (no longer stored individually in the repository)
...
1. Regenerate `pg766.txt` and `cleaned_chapters/` — no longer committed, see [Corpus Reproducibility Script](#corpus-reproducibility-script): `python scripts/prepare_corpus.py --cache pg766.txt --output cleaned_chapters` (needs no network access; reproduces both byte-for-byte).
2. Open `MLASS4v2.ipynb` — this is the notebook with the final RNN/GRU/LSTM comparison and results in this README. `MLASS4.ipynb` is an earlier draft (data exploration and n-gram experiments only; no RNN/GRU/LSTM training) and also depended on the no-longer-committed `cleaned_data/cleaned_text.txt` — see [Corpus Reproducibility Script](#corpus-reproducibility-script) for why that file has no regeneration path.
...
`pg766.txt`, `cleaned_chapters/` (64 files) and `cleaned_data/cleaned_text.txt` were removed from this repository (`A-33`) once `scripts/prepare_corpus.py` — a standard-library-only script — was verified to reproduce the first two byte-for-byte. It resolves the corpus from a pinned, tracked snapshot (`scripts/pg766_source.txt`) rather than a live download by default, so regeneration needs no network access and cannot silently drift from what this repository was actually built and trained on (Project Gutenberg has since revised the text it serves for this eBook ID). Its cleaning step hand-rolls, and verifies against, the notebook's `contractions`/`num2words`/`BeautifulSoup`-dependent pipeline without importing any of the three. `cleaned_data/cleaned_text.txt` is a byproduct of a different, early-draft pipeline (`MLASS4.ipynb`, not the notebook that trains the models) and is deliberately not reproduced by this script — see [`scripts/README.md`](scripts/README.md) for the full detail on all of the above, including exactly how the stdlib-only cleaning step was derived and verified.
...
├── scripts/
│   ├── prepare_corpus.py    # Regenerates pg766.txt + cleaned_chapters/ (see Corpus Reproducibility Script)
│   ├── pg766_source.txt     # Pinned 2024 snapshot the script regenerates from — no network access needed
│   └── README.md            # Full detail on the regeneration script and what it reproduces
...
`pg766.txt`, `cleaned_chapters/` and `cleaned_data/` are no longer committed — see [Corpus Reproducibility Script](#corpus-reproducibility-script).
...
- **Previously shipped pickle artefacts (remediated 2026-07-29):** this repository formerly committed five `.pickle` files (`tokenizer.pickle`, `tok.pickle`, `tok_model_4.pickle`, `word_to_index.pickle`, `index_to_word.pickle`). Pickle deserialisation executes arbitrary code by design, and this is a public repository, so all five were removed rather than kept or blindly converted. To regenerate safe equivalents: re-run the Model 5 tokenizer-fitting cell in `MLASS4v2.ipynb` against a regenerated `cleaned_chapters/` (see [Corpus Reproducibility Script](#corpus-reproducibility-script)) (this does not require retraining the neural network itself), then export with Keras's `tokenizer.to_json()` for `tokenizer.json`, and dump `tokenizer.word_index` as `vocab.json`. This was not done as part of this documentation pass, to avoid deserialising the removed pickles or fabricating their contents.
- **No committed `requirements.txt` or environment file** — dependencies must be reconstructed from the notebooks' `import` cells.
- **Corpus no longer committed (2026-08-18, `A-33`):** `pg766.txt`, `cleaned_chapters/` and `cleaned_data/cleaned_text.txt` were removed and replaced with `scripts/prepare_corpus.py` + the pinned `scripts/pg766_source.txt`, once the script was verified to reproduce `pg766.txt` and all 64 `cleaned_chapters/` files byte-for-byte. `cleaned_data/cleaned_text.txt` has **no regeneration path** and none was built — it turns out to be a byproduct of `MLASS4.ipynb`'s separate, NLTK-dependent (`stopwords` + `WordNetLemmatizer`) cleaning pipeline, not of `MLASS4v2.ipynb`'s `clean_and_preprocess` → `cleaned_chapters/` pipeline that trains and evaluates the RNN/GRU/LSTM models — confirmed directly, not assumed: `MLASS4v2.ipynb` never reads `cleaned_data/` at all, and the 64 cleaned chapters joined together do not match the committed `cleaned_data/cleaned_text.txt`, even in length. Nothing this README documents as working depends on it.
